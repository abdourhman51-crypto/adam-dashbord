...
- **2026-08-13 — LIVE BUG FIXED on W1 (real traffic, not a STEP): the paid/agent replies were failing — either `NodeOperationError: invalid syntax` (executions #6359, #6372, #6373) or a silent empty reply that fell back to the canned "هذه تستحق جواباً أدقّ ممّا كنت سأقول…" rescue message (#6382, #6383, the founder's ID#6381 screenshot). REAL ROOT CAUSE: `enableStreaming` defaulted to `true` on the v3.1 AI Agent nodes.**
  - **A WRONG diagnosis was tried first, and is recorded here as a warning.** Initial theory: the node's `options.systemMessage` used an ES6 backtick template literal and that was breaking the LangChain ToolsAgent. It was rewritten to a plain single-quoted string and published. **This did NOT fix anything** — execution #6373 ran at 13:18 *after* that publish, showed the new single-quoted systemMessage in its node snapshot, and *still threw the identical `invalid syntax`*. The backtick was never the cause. The rewrite is harmless (byte-identical prompt) and was left in place, but it is not the fix. Lesson: do not ship a fix on circumstantial evidence without a mechanism — verify against a real post-deploy execution before claiming resolution.
  - **The real mechanism, schema-confirmed:** `@n8n/n8n-nodes-langchain.agent` v3.1 has an `options.enableStreaming` parameter whose **default is `true`**. `paid aget adam` (and `HR - Compose`) set only `systemMessage`, so streaming was on by default. W1 is driven by a **Telegram webhook**, not a Chat Trigger that consumes a token stream — so the streamed output never lands in `$json.output`. `Gate - Agent Reply` and `FA - Send Reply1` both read `$('paid aget adam').first().json.output`; with it empty, the gate returns `{blocked:true, violations:["empty"], fallback_key:"rescue"}` and FA sends the hardcoded rescue line. In a setup edge the same streaming path throws `invalid syntax` instead. Both symptoms, one cause. This matches the founder's "اصبح يخرف" (recently started malfunctioning) — a platform default-flip to streaming-on on the v3.1 node, not anything the founder or a migration changed. Older followers have 30–170 saved AI replies in `n8n_chat_histories` (agent worked before); the failing follower 7377091520 has zero.
  - **Fix:** set `options.enableStreaming = false` on both `paid aget adam` and `HR - Compose` (atomic `setNodeParameter`, systemMessage untouched). Verified the params landed and published — `activeVersionId` now matches the draft.
  - **Deploy pitfall re-confirmed — important for W1:** `W1` (`42loY0bgUSwYmHFV`) is `active: true`; `update_workflow` writes only the *draft*, and the change does NOT reach live traffic until an explicit `publish_workflow`. This is exactly why the first (wrong) fix's error still recurred at #6373 before publish. Always publish, then verify `activeVersionId == versionId`.
  - **Verification status at handoff:** params confirmed on the live version; a real inbound message still needs to confirm a genuine agent reply now appears (streaming off → `$json.output` populated → gate passes). No real execution has hit the streaming-off version yet at time of writing.
  - **Not touched:** no SQL. W3 untouched (`active: false`).
