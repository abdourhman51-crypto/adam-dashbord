...
- **2026-08-13 — LIVE BUG FIXED on W1 (real traffic, not a STEP): `paid aget adam` was intermittently throwing `NodeOperationError: invalid syntax`, dropping real paid parents' replies (executions #6342-adjacent, #6359, and a 3rd instance #6372 caught mid-fix).**
  - **Root cause diagnosed, not fully proven mechanistically — this distinction matters, see below.** The node's `options.systemMessage` used an ES6 backtick template literal (`` ={{ `<static Arabic prompt>` + (...) }} ``) — this is the one node in the whole product built that way; every other node this session used plain-string concatenation. The failure was intermittent (not universal), always on the same follower/text pattern, and only reproduced under real Postgres-memory replay (`test_workflow`'s simulated/empty memory never triggered it). That circumstantial pattern pointed at the backtick construct interacting badly with n8n's LangChain ToolsAgent memory-replay path, but the exact internal mechanism was never isolated line-by-line inside `executeBatch.ts`.
  - **Fix:** rewrote `options.systemMessage` as a plain single-quoted string (all real newlines escaped to `\n`) concatenated with the `family_context` append — same final prompt text, byte-for-byte, different JS construction. No wording, no logic, nothing else on the node changed.
  - **Deploy pitfall caught live — important for future n8n work on this workflow:** the first `update_workflow` call only wrote the *draft*. `W1` (`42loY0bgUSwYmHFV`) is `active: true`, and its `activeVersionId` stayed pointed at the old version until an explicit `publish_workflow` call was made — draft-save and publish are separate steps here exactly like the founder's contract warns, and this gap is why the identical error recurred (execution #6372) *after* the draft fix was saved but *before* it was published. Confirmed directly: both #6359 and #6372's execution snapshots still show the old backtick text. Published now; `activeVersionId` confirmed to match the draft's `versionId` immediately after.
  - **Not independently verified beyond publishing** — no real message has hit the fixed, live version yet at time of writing. Watching for the next real execution on this node to confirm no recurrence.
  - **Not touched:** no SQL, no other node, W3 untouched (`active: false`).
